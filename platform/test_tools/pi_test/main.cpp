...
void makePiTestWheelTargets(
  ...
  switch (mode) {
    ...
    case MOTION_RIGHT:
      for (int i = 0; i < NUM_WHEELS; i++) {
        wheelTargets[i] =
          (float)STRAFE_SIGN[i] * (float)testRpm;
      }
      break;
...
void applyDpadMotion() {
  ...
  float wheelTargets[NUM_WHEELS];
  makePiTestWheelTargets(activeMotion, wheelTargets);
  setWheelTargets(wheelTargets);
}

void sendPiConfig() {
  String message = "CFG K";
  message += String((int)lroundf(speedKp * 1000.0f));
  message += " I";
  message += String((int)lroundf(speedKi * 1000.0f));
  message += " R";
  message += String(testRpm);
  message += " C";
  message += String(runtimeCurrentLimit);

  sendTextToPi(message);

  Serial.print("PI CONFIG Kp=");
  Serial.print(speedKp, 3);
  Serial.print(" Ki=");
  Serial.print(speedKi, 3);
  Serial.print(" rpm=");
  Serial.print(testRpm);
  Serial.print(" current=");
  Serial.println(runtimeCurrentLimit);
}
...
void handleTestButton(uint8_t buttonId) {
  ...
  switch (buttonId) {
    ...
    case 6:  // L2: Kp down
      speedKp = clampFloat(speedKp - KP_STEP, 0.0f, 10.0f);
      resetAllPi();
      changed = true;
      break;
    ...
    case 3:  // SQUARE: Ki down
      speedKi = clampFloat(speedKi - KI_STEP, 0.0f, 5.0f);
      resetAllPi();
      changed = true;
      break;

    case 2:  // TRIANGLE: Ki up
      speedKi = clampFloat(speedKi + KI_STEP, 0.0f, 5.0f);
      resetAllPi();
      changed = true;
      break;

    case 11:  // L3: current limit down
      runtimeCurrentLimit = clampInt16(
        runtimeCurrentLimit - 500,
        500,
        HARD_CURRENT_LIMIT
      );
      resetAllPi();
    ...
    case 1:  // CIRCLE: integral reset
      resetAllPi();
      sendTextToPi("PI INTEGRAL RESET");
      break;

    case 9:  // OPTIONS: status
      sendPiConfig();
      break;

    case 13:  // TOUCHPAD: Kp/Ki initial reset
      speedKp = 0.50f;
      speedKi = 0.00f;
      resetAllPi();
      changed = true;
...
void sendModeTelemetry() {
  ...
  Serial.print(" temp=");
  Serial.println(
    motorTemperature[WHEEL_TO_MOTOR[wheelIndex]]
  );
}
...
void printModeSetup() {
  Serial.println("IM920 + PS5 DPAD PI TUNING TEST");
  Serial.println();
  Serial.println("DPAD UP/DOWN  : FWD/BWD fixed rpm");
  Serial.println("DPAD RIGHT/LEFT: strafe fixed rpm");
  Serial.println("L1/R1         : test rpm -/+200");
  Serial.println("L2/R2         : Kp -/+0.10");
  Serial.println("SQUARE/TRIANGLE: Ki -/+0.02");
  Serial.println("L3/R3         : current limit -/+500");
  Serial.println("CIRCLE        : PI integral reset");
  Serial.println("OPTIONS       : send current settings");
  Serial.println("TOUCHPAD      : reset Kp=0.50 Ki=0.00");
  Serial.println();
  Serial.print("Initial Kp=");
  Serial.print(speedKp, 2);
  Serial.print(" Ki=");
  Serial.print(speedKi, 2);
  Serial.print(" rpm=");
  Serial.print(testRpm);
  Serial.print(" current=");
  Serial.println(runtimeCurrentLimit);
